...
pub fn verify(params: &Parameters<super::engines::Bn256>, vk: &VK<super::engines::Bn256>, proof: &Proof, inputs: &[Num<crate::engines::bn256::Fr>]) -> bool {
    let instances = inputs.iter().cloned().map(num_to_halo_fp).collect_vec();

    let mut transcript = <EvmTranscript::<halo2_curves::bn256::G1Affine, _, _, _> as TranscriptReadBuffer<_, _, _>>::init(Cursor::new(proof.0.clone()));
